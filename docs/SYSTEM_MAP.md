...
* **堅牢化:** 同期処理は **`DB::afterCommit` 内で実行**し、Firebase側の通信遅延やエラーが、DB側の決済・発券処理をロールバック（道連れ）にしない設計を徹底する。
* **注意:** 同期失敗時は `try-catch` による Silent Catch を行い、ログ出力を欠かさないこと。
...
### 6. API コンテキスト・フィルタリング義務

* **鉄則:** 同一テーブル（orders等）を異なる用途（グッズ/チケット）で共有する場合、必ず `type` 等のパラメータによる API レベルのフィルタリングを実装すること。
* **理由:** クライアント側のロジックに頼らず、サーバーサイドで表示コンテキストを厳密に分離することで、データの表示不整合（地雷）を根絶する。
