...
* **ライブ情報の閲覧・検索・購入**: Stripe決済によるセキュアで即時性の高いチケット購入フロー。
* **スマート入場システム**: 巨大QRコード表示、リアルタイム入場判定、Haptic Feedback（振動）による直感的な完了通知。
* **チケット譲渡・管理**: 購入済みチケットの分配やステータス管理。
...
* **レアリティ演出**: 排出時の演出や、アイテムごとのレアリティ（UR/SSR/SR/R）設定。
※ 現在の実装状況について  
・排出ロジック、所持管理、装備反映フローは実装済み  
・ガチャ演出（アニメーション・SE）および複数アイテムの量産は今後の拡張予定  
・現時点ではデフォルトアイテムのみを用いた最小構成で動作確認を行っています  

本機能は、演出やアイテム追加を容易に拡張できる設計となっています。

### 💬 コミュニティチャット

* **イベント連動ロビー**: イベントごとの掲示板でファン同士が交流。
* **スレッド（ルーム）作成**: ポイント消費（50pt）によるユーザー主導のコミュニティ形成。
* **リアルタイムチャット**: 削除、リプライ、リアクション、メンション機能完備。
* **ハイブリッド課金モデル**:
* 各ルーム 10回まで送信無料。
* 以降は 1メッセージにつき 5pt 消費（スパム防止・収益化）。
* 連投制限（Throttling）による荒らし対策。



### 💎 ポイント & サブスクリプション

* **ポイント経済圏**: ユーザー所持ポイントの管理（DB排他制御実装済み）と、チャット・ガチャ等への利用。
* **取引履歴ログ**: すべてのポイント消費・獲得アクションを汎用ログとして記録。
* **サブスクリプション**: Stripe連携による月額プラン契約（加入・更新・アップグレード・解約）。

### 🛡️ カスタマーサポート (Support)

* **対話型チャットUI**: ユーザーと運営がLINE風の画面でスムーズに質疑応答。
* **コンテキスト連携**: 特定の「注文」や「イベント」を紐付けて問い合わせ可能（ポリモーフィック関連）。
* **データライフサイクル**: 解決済みデータの保存期限管理と、バッチ処理による自動削除。

### 🛠 アーティスト/運営管理 (Admin)

* **CMS機能**: イベント・グッズ・お知らせ・ガチャ内容の入稿管理。
* **入場/引換スキャナー**: 権限チェック付きのQR読み取りカメラ内蔵（スタッフ専用モード）。
* **ダッシュボード**: 売上・入場者数・アクティブユーザーのリアルタイム分析 (**Laravel Filament**)。
* **公式アカウント権限管理**:
    * なりすまし防止のため、**独自のプロフィール画像アップロードはアーティスト/運営のみに限定**（RBAC制御）。
    * 一般ユーザーはガチャで入手した公認アイコンのみ使用可能にすることで、コミュニティの安全性と著作権保護を両立。
